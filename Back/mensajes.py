...
#! Tentativamente cambiar parametros por Copy o agregar copy
def execute(self, target_nodo, source, copy, parametros, prioridad, daemon_id, tipo_daemon):
    if tipo_daemon == 1:
        mensaje(self,
                "EXECUTE",
                self.id,
...
def startTimer(self, internal_target, internal_source, timer_value=1):
    ...
    newEvent = Mensaje("TIMER",
                       ...
                       None, None, None,
                       None, None, None,
                       None  # ! CORRESPONDE A LA LISTA DE FALLOS
                       )  # port=0
...
def startTimerClone(self, timer_value, tipo_daemon, clone_id, daemon_id):
    newEvent = Mensaje("TIMER",
                       ...
                       clone_id,
                       None,
                       None, None, None, None # ! CORRESPONDE A LA LISTA DE FALLOS
                       )
    self.transmit(newEvent)


# Manda T1 a nodo target
def invokeTask(self, target, operacion, archivo, daemon_id):
    if operacion == "STORE":
        mensaje(self,
                "STORE",
                target,
...
def store(self, target, parametros=None, archivo=None):
    # Viene del cliente
...
def report(self, result, daemon_id, parameters, prioridad=None, operacion=None, nodo_objetivo=None, archivo = None):
    if result == "FAILURE":
        mensaje(self,
                "REPORT",
                self.id,
